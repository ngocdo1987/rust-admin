...
async fn main() -> std::io::Result<()> {
    // The formal environment can remove the log display
    std::env::set_var("RUST_LOG", "actix_web=info"); //The formal environment can comment on this trip ***
    env_logger::init(); //The formal environment can comment on this trip ***

    let setting = &*config::SETTING;
    let info = &setting.app;
    let conn_string = config::get_conn_string();
    db::init_connections(&conn_string); //Database initialization
    let host_port = &format!("{}:{}", &info.host, &info.port); //Address/port
    ...
    HttpServer::new(move || {
        let mut tpl = tmpl!("/templates/**/*"); //Template engine
        ...
        App::new()
            ...
            .data(tpl)
            .data(server.clone())
            .wrap(middleware::Logger::default()) //The formal environment can comment on this trip ***
            .service(Files::new("/static", "public/static/")) //Static file directory
            .service(Files::new("/upload", "public/upload/")) //Upload file directory
